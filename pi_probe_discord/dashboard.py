...
from .baselines import average, history_points_for_window, min_max
...
def generate_premium_dashboard(history: dict[str, list[dict[str, Any]]], now: datetime, chart_path: str, speed_result: SpeedResult) -> tuple[bool, str]:
    ...
    figure.text(0.04, 0.95, "Pi Probe NBN Performance Dashboard", color="#f8fafc", fontsize=26, fontweight="bold", ha="left", va="top")
    figure.text(
        0.04,
        0.915,
        f"Recent local history · {stats['start']} – {stats['end']} · {stats['tests']} tests",
        color="#94a3b8",
        fontsize=13,
        ha="left",
        va="top",
    ...
    cards = [
        ("Internet quality score", f"{score}/100", "Composite: speed, upload, ping, reliability"),
        ("Median download", f"{stats['medianDown']:.0f} Mbps", f"Average {stats['avgDown']:.0f} Mbps"),
        ("Upload average", f"{stats['avgUp']:.1f} Mbps", "Stable upstream performance"),
        ("Average ping", f"{stats['avgPing']:.1f} ms", "Latency profile"),
        ("Tests ≥250 Mbps", f"{stats['pct250']:.0f}%", f"{stats['pct300']:.0f}% at or above 300 Mbps"),
    ]
    for idx, (title, value, subtitle) in enumerate(cards):
        ax = figure.add_subplot(gs[0, idx * 12 // 5 : (idx + 1) * 12 // 5])
        ax.set_facecolor("#152033")
        ax.set_xticks([])
        ax.set_yticks([])
        for spine in ax.spines.values():
            spine.set_visible(False)
        ax.text(0.05, 0.72, title, transform=ax.transAxes, color="#cbd5e1", fontsize=12, fontweight="bold", ha="left")
        ax.text(0.05, 0.38, value, transform=ax.transAxes, color="#f8fafc", fontsize=28, fontweight="bold", ha="left")
        ax.text(0.05, 0.16, subtitle, transform=ax.transAxes, color="#94a3b8", fontsize=11, ha="left")

    def style_axis(ax: Any, title: str) -> None:
        ax.set_facecolor("#152033")
        ax.set_title(title, loc="left", color="#f8fafc", fontsize=17, fontweight="bold", pad=12)
        ax.grid(color="#334155", linewidth=0.8, alpha=0.65)
    ...
    ax_band = figure.add_subplot(gs[1, :7])
    style_axis(ax_band, "Daily download reliability band")
    ...
    ax_band.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))
    ...
    ax_heat = figure.add_subplot(gs[1, 7:])
    style_axis(ax_heat, "Speed heatmap by hour and weekday")
    ...
    ax_times = figure.add_subplot(gs[2, :6])
    style_axis(ax_times, "Best and slowest times of day")
    ...
    ax_hist = figure.add_subplot(gs[2, 6:9])
    style_axis(ax_hist, "Download distribution")
    ax_hist.hist([row.download for row in rows if row.download is not None], bins=24, color="#38bdf8", alpha=0.75)
    ax_hist.axvline(stats["medianDown"], color="#38bdf8", linestyle="--", linewidth=1.5)
    ax_hist.axvline(stats["avgDown"], color="#94a3b8", linestyle=":", linewidth=1.5)
    ax_hist.set_xlabel("Mbps", color="#cbd5e1")
    ax_hist.set_ylabel("Tests", color="#cbd5e1")

    ax_exec = figure.add_subplot(gs[2, 9:])
    ax_exec.set_facecolor("#152033")
    ax_exec.set_xticks([])
    ax_exec.set_yticks([])
    for spine in ax_exec.spines.values():
        spine.set_visible(False)
    ax_exec.text(0.04, 0.92, "Executive readout", transform=ax_exec.transAxes, color="#f8fafc", fontsize=17, fontweight="bold", ha="left")
    ...
    lines = [
        f"• Most typical speed: {stats['medianDown']:.0f} Mbps down",
        f"• Reliability floor: {stats['p05']:.0f} Mbps at 5th percentile",
        f"• Peak band: {stats['p95']:.0f} Mbps at 95th percentile",
        ...
        f"• Latency profile: {stats['avgPing']:.1f} ms average ping",
        f"• Latest snapshot: {_format_metric(speed_result.download_mbps, 'Mbps')} ↓ / {_format_metric(speed_result.upload_mbps, 'Mbps')} ↑ / {_format_metric(speed_result.ping_ms, 'ms', 0)}",
    ]
    for idx, line in enumerate(lines):
        ax_exec.text(0.06, 0.80 - idx * 0.10, line, transform=ax_exec.transAxes, color="#cbd5e1", fontsize=12.5, ha="left")

    figure.text(
        0.04,
        0.025,
        "Interpretation: shaded bands show normal variation; heatmap exposes recurring congestion windows; KPI score is a local summary, not an ISP benchmark.",
        color="#94a3b8",
        fontsize=10,
        ha="left",
    )
